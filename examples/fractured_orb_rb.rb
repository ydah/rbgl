...
shader = RLSL.define(:fractured_orb_ruby) do
  ...
  helpers do
    ...
    def icosahedronVertex(p)
      ...
      v = vec3(1.0, 0.0, PHI) if test1 > dot1
      test2 = ap.z + (ap.y * PHI)
      dot2 = (ap.x * v.x) + (ap.y * v.y) + (ap.z * v.z)
      v = vec3(0.0, PHI, 1.0) if test2 > dot2
      bs = boolSign3(p)
    ...
    def map_scene(p, anim_time)
      ...
      while i < 4.0
        ...
        wobble = sin((expImpulse(t2,
                                 20.0) * 2.2) + (pow(3.0 * t2, 1.5) * 2.0 * TAU) - PI) * smoothstep(0.4, 0.0, t2) * 0.2
        anim = wobble + explode
      ...
      d = min(d, bound) if bound * scale > 0.002
    ...
    def light_func(origin, rayDir, envOrientation)
      ...
      if abs(denom) >= 0.0001
        ...
        if t >= 0.0
          ...
          l *= smoothstep(6.0, 0.0, length(uv_l))
          result = vec3(l, l, l)
        end
      end
    ...
    def env_func(origin, rayDir, envOrientation)
      ...
      sphericalMatrix_mul(envOrientation, origin)
  ...
  fragment do |frag_coord, resolution, u|
    ...
    while disperse < maxDisperse
      ...
      wavelength += (((rand_val * 2.0) - 1.0) * 0.1)
      ...
      while bounce < 5.0
        ...
        extinctionDist += hitLen if invert < 0.0

        break if hitResY < 0.001
      ...
      if bounceCount < 0.5
        ...
        col = vec3(col.x + (sam.x * maxDisperse / 2.0), col.y + (sam.y * maxDisperse / 2.0),
                   col.z + (sam.z * maxDisperse / 2.0))
        break
      else
